...
writer = SummaryWriter(log_dir="logsseq")  # Specify the log directory
...
try: 
    # load the model
    checkpoint = torch.load("modelseq_0.pth")
...
for epoch in range(num_epochs):
    ...
    for iteration, waveform in enumerate(dataloader):
        ...
        # Compute the loss
        loss = criterion(decoder_output, codec_output).sum(-1).mean()
        # Backward pass
        loss.backward()
        # Update weights
        optimizer.step()
